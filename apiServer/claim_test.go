...
func TestClaims(t *testing.T) {
	...
	**	Test cases. Set claims
	...
	testCasesSet := []struct {
		...
		{
			name: "valid - set claim from uid#" + strconv.Itoa(user1.Uid) + " and uid#" + strconv.Itoa(user2.Uid),
			uid:  user1.Uid,
			payload: map[string]interface{}{
			...
			name: "valid - set claim from uid#" + strconv.Itoa(user2.Uid) + " and uid#" + strconv.Itoa(user1.Uid),
			uid:  user2.Uid,
			payload: map[string]interface{}{
			...
			name: "valid - set claim from uid#" + strconv.Itoa(user1.Uid) + " and uid#" + strconv.Itoa(user3.Uid),
			uid:  user1.Uid,
			payload: map[string]interface{}{
			...
			name: "valid - set claim from uid#" + strconv.Itoa(user2.Uid) + " and uid#" + strconv.Itoa(user3.Uid),
			uid:  user2.Uid,
			payload: map[string]interface{}{
				"otherUid": float64(user3.Uid),
			},
	...
	for _, tc := range testCasesSet {
		t.Run(tc.name, func(t_ *testing.T) {
			...
			// start test
			server.ClaimSet(rec, req.WithContext(ctx))
			if rec.Code == tc.expectedStatus && tc.expectedStatus == http.StatusOK {
				t_.Logf(GREEN_BG + "SUCCESS: claim was set" + NO_COLOR)
	...
	**	Test cases. Get claims users
	...
	for _, tc := range testCasesGet {
		t.Run(tc.name, func(t_ *testing.T) {
			...
			// start test
			server.UserGetClaimed(rec, req.WithContext(ctx))
	...
	**	Test cases. Unset claims
	...
	testCasesUnset := []struct {
		...
		{
			name: "valid - unset claim from uid#" + strconv.Itoa(user1.Uid) + " and uid#" + strconv.Itoa(user2.Uid),
			uid:  user1.Uid,
			payload: map[string]interface{}{
				"otherUid": float64(user2.Uid),
			},
			...
			name: "valid - unset claim from uid#" + strconv.Itoa(user2.Uid) + " and uid#" + strconv.Itoa(user1.Uid),
			uid:  user2.Uid,
			payload: map[string]interface{}{
				"otherUid": float64(user1.Uid),
			},
			...
			name: "valid - unset claim from uid#" + strconv.Itoa(user1.Uid) + " and uid#" + strconv.Itoa(user3.Uid),
			uid:  user1.Uid,
			payload: map[string]interface{}{
				"otherUid": float64(user3.Uid),
			},
			...
			name: "valid - unset claim from uid#" + strconv.Itoa(user2.Uid) + " and uid#" + strconv.Itoa(user3.Uid),
			uid:  user2.Uid,
			payload: map[string]interface{}{
				"otherUid": float64(user3.Uid),
			},
	...
	for _, tc := range testCasesUnset {
		t.Run(tc.name, func(t_ *testing.T) {
			...
			// start test
			server.ClaimUnset(rec, req.WithContext(ctx))
			if rec.Code == tc.expectedStatus && tc.expectedStatus == http.StatusOK {
				t_.Logf(GREEN_BG + "SUCCESS: claim was set" + NO_COLOR)
			} else if rec.Code == tc.expectedStatus {
				t_.Logf(GREEN_BG + "SUCCESS: test was failed as it expected" + NO_COLOR)
