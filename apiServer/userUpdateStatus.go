...
func (server *Server) userUpdateStatus(w http.ResponseWriter, r *http.Request) {
	...
	server.Log(r, "/user/update/status/", message)

	err = json.NewDecoder(r.Body).Decode(&request)
	if err != nil {
		server.LogError(r, "/user/update/status/", "request json decode failed - "+err.Error())
	...
	if !isExist {
		server.LogError(r, "/user/update/status/", "x-reg-token not exist in request")
	...
	if !ok {
		server.LogError(r, "/user/update/status/", "x-reg-token has wrong type")
		server.error(w, errDef.InvalidArgument.WithArguments("Поле x-reg-token имеет неверный тип", "x-reg-token field has wrong type"))
		return
	}

	if token == "" {
		server.LogError(r, "/user/update/status/", "x-reg-token is empty")
	...
	if err != nil {
		server.LogWarning(r, "/user/update/status/", "TokenMailDecode returned error - "+err.Error())
		server.error(w, errDef.UserNotLogged)
		return
	}
	...
	if errDef.RecordNotFound.IsOverlapWithError(err) {
		server.LogWarning(r, "/user/update/status/", "GetUserByMail - record not found")
		server.error(w, errDef.UserNotExist)
		return
	} else if err != nil {
		server.LogWarning(r, "/user/update/status/", "GetUserByMail returned error - "+err.Error())
	...
	if err != nil {
		server.LogWarning(r, "/user/update/status/", "UpdateUser returned error - "+err.Error())
		server.error(w, errDef.DatabaseError)
		return
	}

	w.WriteHeader(http.StatusOK) // 200
	server.LogSuccess(r, "/user/update/status/", "user #"+BLUE+strconv.Itoa(user.Uid)+NO_COLOR+
...
func (server *Server) HandlerUserUpdateStatus(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "PATCH" {
		...
		server.Log(r, "/user/update/status/", "client wants to know what methods are allowed")
	} else {
		// ALL OTHERS METHODS
		server.LogWarning(r, "/user/update/status/", "wrong request method")
