...
func (server *Server) wsWriteErrorMessage(r *http.Request, ws *websocket.Conn, messageBody string) error {
	...
	if err != nil {
		server.LogError(r, "/ws/auth/", "ws.WriteMessage returned error - "+err.Error())
		return nil
	}
	return nil
}
	...
	for {
		...
		if err != nil {
			...
			if ok && (closeErr.Code == 1000 || closeErr.Code == 1001) {
				server.LogWarning(r, "/ws/auth/", closeErr.Error())
			} else {
				server.LogError(r, "/ws/auth/", "ws.ReadMessage returned error - "+err.Error())
			}
			return
		}
		err = json.Unmarshal(RequestMessage, &decodedMessage)
		if err != nil {
			server.LogError(r, "/ws/auth/", "request json decode failed - "+err.Error()+`. Skip request`)
			err = server.wsWriteErrorMessage(r, ws, "request json decode failed")
			if err != nil {
				server.LogError(r, "/ws/auth/", "wsWriteErrorMessage returned error - "+err.Error())
				return
			}
			continue
		}
		...
		if !isExists {
			server.LogWarning(r, "/ws/auth/", `"uidReceiver" not exist in received by ws message. Skip request`)
		...
		if !ok {
			server.LogWarning(r, "/ws/auth/", `wrong type of "uidReceiver". Skip request`)
		...
		server.Log(r, "/ws/auth/", "user #"+BLUE+strconv.Itoa(uid)+NO_COLOR+" ("+BLUE+r.Host+NO_COLOR+
			") wants to send message to user #"+BLUE+strconv.Itoa(uidReceiver)+NO_COLOR)
		arg, isExists = decodedMessage["body"]
		if !isExists {
			server.LogWarning(r, "/ws/auth/", `"body" not exist in received by ws message. Skip request`)
		...
		if !ok {
			server.LogWarning(r, "/ws/auth/", `wrong type of "body". Skip request`)
		...
		if !isExists {
			server.LogWarning(r, "/ws/auth/", `user #`+BLUE+strconv.Itoa(uidReceiver)+NO_COLOR+
		...
		if err != nil {
			server.LogWarning(r, "/ws/auth/", `SetNewMessage returned error - `+err.Error())
			return
		}
		err = server.session.SendMessageToLoggedUser(uidReceiver, uid, messageBody)
		if err != nil {
			server.LogWarning(r, "/ws/auth/", `SendMessageToLoggedUser returned error - `+err.Error())
			return
		}
		server.LogSuccess(r, "/ws/auth/", "message from user #"+BLUE+strconv.Itoa(uid)+NO_COLOR+
...
func (server *Server) WebSocketHandlerAuth(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		server.LogError(r, "/ws/auth/", "uid is invalid "+err.Error())
	...
	server.Log(r, "/ws/auth/", "Request was recieved, uid="+BLUE+strconv.Itoa(uid)+NO_COLOR+
		" ws-auth-token="+BLUE+wsAuthToken+NO_COLOR)

	upgrader.CheckOrigin = func(r *http.Request) bool { return true }
	ws, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		server.LogError(r, "/ws/auth/", "upgrader returned error - "+err.Error())
		return
	}
	if uid < 1 || wsAuthToken == "" {
		server.LogWarning(r, "/ws/auth/", "wrong uid or ws-auth-token is empty")
		ws.Close()
		return
	}
	tokenWS, err := server.session.GetTokenWS(uid)
	if err != nil {
		server.LogError(r, "/ws/auth/", "GetTokenWS returned error - "+err.Error())
		ws.Close()
		return
	}
	if tokenWS != wsAuthToken {
		server.LogWarning(r, "/ws/auth/", "ws-auth-token is wrong! Close Web Socket for user #"+
	...
	server.LogSuccess(r, "/ws/auth/", "WebSocket was created")

	server.wsReader(r, ws, uid)

	userSessionWasClosed, err := server.session.RemoveWSConnection(uid, ws)
	if err != nil {
		server.LogWarning(r, "/ws/auth/", "RemoveWSConnection returned error: "+err.Error())
	} else {
